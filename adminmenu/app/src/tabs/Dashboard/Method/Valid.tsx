...
const Valid = ({ method, cleanLogfiles }: { method: MethodProps; cleanLogfiles: (modulId: string) => () => void }) => {
  return (
    <div key={method.mollie.id} style={{ flexBasis: '33%' }}>
      <div className="m-2 p-2 border-b">
        {method.paymentMethod && method.duringCheckout && !method.allowDuringCheckout && (
          <FontAwesomeIcon
            className={'mr-4 ml-1 cursor-help'}
            icon={faExclamationTriangle}
            color={'red'}
            onClick={() =>
              alert(
                'Zahlung vor Bestellabschluss wird nicht unterstützt. Diese Zahlungsart wird nicht zur auswahl stehen.'
              )
            }
        ...
        <div className="float-right">
            ...
            <>
              {method.components === 'S' || method.components === 'Y' ? (
                <FontAwesomeIcon
                  ...
                  onClick={() =>
                    alert(
                      'Mollie Components sind aktiviert: ' +
                        (method.components === 'S' ? 'optional.' : 'obligatorisch.')
                    )
                  }
                  title={
                    'Mollie Components enabled.' + (method.components === 'S' ? ' (optional)' : ' (obligatorisch)')
                  }
          ...
          <FontAwesomeIcon
            ...
            onClick={() =>
              alert(
                method.shipping && method.shipping?.length
                  ? 'Mit folgende Versandarten verknüpft:\n\n' +
          ...
          {method.log && method.paymentMethod ? (
            <FontAwesomeIcon
              onClick={cleanLogfiles(method.paymentMethod.cModulId)}
              icon={faEraser}
              title="Clean Logfiles"
              className="cursor-pointer ml-1 text-red-600"
            />
          ) : null}
          {method.paymentMethod === false ? null : (
            <a title="Einstellungen" href={method.settings} className={'ml-1'} target="_blank" rel="noreferrer">
